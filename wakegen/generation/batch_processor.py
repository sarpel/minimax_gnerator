...
class BatchProcessor:
    ...
    async def _process_single_task(
        ...
        elif getattr(provider, "is_commercial", False) or provider.provider_type.value == "minimax":
        ...
        async def _generate_with_retry() -> GenerationResult:
            ...
            try:
                ...
                if self.config.caching_enabled:
                    cached_path = cache.get(params.text, params.voice_id, provider.provider_type.value)
                ...
                if cached_path:
                    ...
                    if self.config.sample_rate:
                        ...
                        await asyncio.to_thread(resample_audio, output_path, self.config.sample_rate)

                    # Update cache
                    if self.config.caching_enabled:
                        cache.put(params.text, params.voice_id, provider.provider_type.value, output_path, copy=True)

                # Create GenerationResult from the generated audio
                result = self._create_generation_result(params, output_path, provider.provider_type)
    ...
    def _create_generation_result(
        self, params: GenerationParameters, output_path: str, provider_type: ProviderType
    ) -> GenerationResult:
