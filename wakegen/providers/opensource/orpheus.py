...
class OrpheusTTSProvider(BaseProvider):
    ...
    def __init__(self, config: Any):
        ...
        self._selected_model_size: str | None = None
    ...
    async def _get_model(self) -> Any:
        ...
        try:
            ...
            logger.info(
                f"Loading Orpheus TTS ({self._selected_model_size}) on {self._device}..."
            )

            # Initialize model
            # Model weights will be downloaded automatically
            self._model = OrpheusModel(
                model_size=self._selected_model_size, device=self._device
            )
    ...
    async def generate(self, text: str, voice_id: str, output_path: str) -> None:
        ...
        try:
            ...
            logger.debug(
                f"Generating with voice={voice_id}, model={self._selected_model_size}"
            )
            ...
            raise ProviderError(f"Orpheus TTS generation failed: {e!s}") from e
    ...
    async def list_voices(self) -> list[Voice]:
        ...
        try:
            ...
            raise ProviderError(f"Failed to list Orpheus TTS voices: {e!s}") from e
    ...
    async def validate_config(self) -> None:
        ...
        try:
            ...
            raise ProviderError(
                f"Orpheus TTS configuration validation failed: {e!s}"
            ) from e
    ...
    try:
        ...
        try:
            await provider.generate(
                "Hello, this is a test of Orpheus TTS provider.", "default", temp_path
            )
