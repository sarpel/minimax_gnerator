...
class StyleTTS2Provider(BaseProvider):
            ...
            if self._use_gpu:
                try:
                    ...
                    if torch.cuda.is_available():
                        ...
                        self._device = "cpu"
                        logger.warning(
                            "No GPU available, StyleTTS 2 will run on CPU (slower)"
                        )
                except ImportError:
                    self._device = "cpu"
            else:
    ...
    async def generate(self, text: str, voice_id: str, output_path: str) -> None:
        ...
        try:
            ...
            raise ProviderError(f"StyleTTS 2 generation failed: {e!s}") from e
    ...
    async def list_voices(self) -> list[Voice]:
        ...
        try:
            ...
            raise ProviderError(f"Failed to list StyleTTS 2 voices: {e!s}") from e
    ...
    async def validate_config(self) -> None:
        ...
        try:
            ...
            raise ProviderError(
                f"StyleTTS 2 configuration validation failed: {e!s}"
            ) from e
    ...
    try:
        ...
        try:
            await provider.generate(
                "Hello, this is a test of StyleTTS 2 provider.", "default", temp_path
            )
