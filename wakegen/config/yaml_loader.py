...
class ProjectConfig(BaseModel):
    ...
    description: str | None = Field(
        default=None, max_length=500, description="Optional description of the project"
    )
...
class ProviderConfig(BaseModel):
    ...
    voices: list[str] = Field(
        default_factory=list, description="List of voice IDs to use from this provider"
    )
    weight: float = Field(
        default=1.0,
...
class GenerationConfig(BaseModel):
    ...
    output_dir: str = Field(
        default="./output", description="Directory to save generated audio files"
    )
    ...
    def validate_wake_words(cls, v: list[str]) -> list[str]:
        ...
        for i, word in enumerate(v):
            ...
            if not word:
                raise ValueError(
                    f"Wake word at index {i} is empty after stripping whitespace"
                )
            if len(word) > 100:
                raise ValueError(
                    f"Wake word '{word[:20]}...' is too long (max 100 chars)"
                )
            cleaned.append(word)
        return cleaned
...
class AugmentationConfig(BaseModel):
    ...
    enabled: bool = Field(default=True, description="Whether to apply augmentation")
...
class ExportConfig(BaseModel):
    ...
    include_manifest: bool = Field(
        default=True, description="Whether to generate a manifest file"
    )
    ...
    def validate_split_ratio(cls, v: list[float]) -> list[float]:
        ...
        if len(v) != 3:
            raise ValueError(
                "split_ratio must have exactly 3 values (train, val, test)"
            )
        ...
        for i, ratio in enumerate(v):
            if not 0.0 <= ratio <= 1.0:
                raise ValueError(
                    f"split_ratio[{i}] must be between 0.0 and 1.0, got {ratio}"
                )
...
class WakegenConfig(BaseModel):
    ...
    augmentation: AugmentationConfig = Field(
        default_factory=AugmentationConfig, description="Augmentation settings"
    )
    export: ExportConfig = Field(
        default_factory=ExportConfig, description="Export/dataset preparation settings"
    )
