...
class MicrophoneSimulator:
    ...
    def apply_microphone_effect(
        ...
        try:
            ...
            raise MicrophoneSimulationError(
                f"Failed to apply microphone effects: {e!s}"
            ) from e
    ...
    def _apply_fft_eq(
        self, audio: np.ndarray[Any, Any], eq_curve_func: Any, sample_rate: int
    ) -> np.ndarray[Any, Any]:
        """
    ...
    def _add_harmonic_distortion(
        self, audio: np.ndarray[Any, Any], distortion_amount: float
    ) -> np.ndarray[Any, Any]:
        """
        ...
        return cast(
            np.ndarray[Any, Any], (audio * (1.0 - mix_ratio)) + (distorted * mix_ratio)
        )

    def _add_microphone_noise(
        self, audio: np.ndarray[Any, Any], noise_floor_db: float
    ) -> np.ndarray[Any, Any]:
        """
    ...
    async def simulate_microphone(
        self, input_path: str, output_path: str, profile: MicrophoneProfile
    ) -> None:
        """
        ...
        try:
            ...
            raise MicrophoneSimulationError(
                f"Failed to simulate microphone: {e!s}"
            ) from e
    ...
    def get_preset_microphone(self, preset_name: str) -> MicrophoneProfile:
        ...
        if preset_name not in presets:
            ...
            raise MicrophoneSimulationError(
                f"Unknown microphone preset '{preset_name}'. Available: {available}"
            )
    ...
    def simulate_microphone_coloration(
        self, signal: np.ndarray[Any, Any], mic_type: str = "cardioid"
    ) -> np.ndarray[Any, Any]:
        """
    ...
    def apply_proximity_effect(
        self, signal: np.ndarray[Any, Any], distance_cm: float = 30.0
    ) -> np.ndarray[Any, Any]:
        """
