...
class DevicePresetManager:
    ...
    def _build_microphone_filter(self, spec: DeviceAudioSpec) -> dict[str, Any]:
        ...
        return {
            ...
            "lowpass_freq": min(
                spec.sample_rate // 2 - 100, spec.frequency_response[1]
            ),
    ...
    def _build_telephony_effects(
        self, spec: DeviceAudioSpec, device: TargetDevice
    ) -> dict[str, Any]:
    ...
    def _get_priority_augmentations(
        self, device: TargetDevice, env: DeviceEnvironment
    ) -> list[str]:
    ...
    def get_presets_for_category(self, category: str) -> list[DevicePreset]:
        ...
        category_devices = {
            ...
            "sbc": [TargetDevice.RPI_USB, TargetDevice.RPI_I2S, TargetDevice.RPI_HAT],
            "smart_speaker": [
                TargetDevice.SMART_SPEAKER_BUDGET,
                TargetDevice.SMART_SPEAKER_MID,
                TargetDevice.SMART_SPEAKER_PREMIUM,
            ],
            "mobile": [TargetDevice.MOBILE_IOS, TargetDevice.MOBILE_ANDROID],
            "embedded": [TargetDevice.JETSON_NANO, TargetDevice.CORAL_DEV],
            "generic": [
    ...
    def get_augmentation_config(
        self, device: TargetDevice, include_all: bool = False
    ) -> dict[str, Any]:
        """
