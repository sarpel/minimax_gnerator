...
class NoiseConfig(BaseModel):
    ...
    probability: float = Field(
        default=0.5, ge=0, le=1, description="Probability of applying"
    )
...
class ReverbConfig(BaseModel):
    ...
    decay: float = Field(
        default=0.5, ge=0.1, le=2.0, description="Decay time in seconds"
    )
...
class PitchConfig(BaseModel):
    ...
    pitch_semitones: float = Field(
        default=1.0, ge=0, le=4, description="Pitch range in semitones"
    )
    speed_percent: float = Field(
        default=5.0, ge=0, le=20, description="Speed variation percent"
    )
...
class DeviceConfig(BaseModel):
    ...
    devices: list[DeviceType] = Field(
        default_factory=list, description="Devices to simulate"
    )
...
class ApplyRequest(BaseModel):
    ...
    copies_per_file: int = Field(
        1, ge=1, le=10, description="Number of augmented copies"
    )
...
class DevicePreset(BaseModel):
    ...
    description: str
    icon: str
    frequency_response: dict[str, Any] | None = None
...
PRESET_PROFILES = {
    "clean": AugmentationProfile(
        ...
        reverb=ReverbConfig(
            enabled=False, room_size=RoomSize.MEDIUM, decay=0.5, wet_dry_mix=0.0
        ),
...
DEVICE_PRESETS = [
    ...
    DevicePreset(
        id="laptop", name="Laptop", description="Built-in laptop microphone", icon="💻"
    ),
    DevicePreset(
        id="speaker",
        name="Smart Speaker",
    ...
    DevicePreset(id="car", name="Car", description="In-vehicle microphone", icon="🚗"),
]
...
@router.get(
    "/devices", response_model=list[DevicePreset], summary="List device presets"
)
...
@router.post("/apply", response_model=ApplyResponse, summary="Apply augmentation")
async def apply_augmentation(
    ...
    background_tasks.add_task(
        run_augmentation, job_id=job_id, request=request, file_count=input_count
    )
            ...
            for i in range(request.copies_per_file):
                ...
                if request.profile.pitch.enabled:
                    ...
                    n_steps = random.uniform(
                        -request.profile.pitch.pitch_semitones,
                        request.profile.pitch.pitch_semitones,
                    )
                    if abs(n_steps) > 0.1:
                        y_aug = librosa.effects.pitch_shift(
                            y_aug, sr=sr, n_steps=n_steps
                        )
...
@router.post("/preview", summary="Preview augmentation")
async def preview_augmentation(
    file_path: str, profile: AugmentationProfile
) -> dict[str, Any]:
    """
