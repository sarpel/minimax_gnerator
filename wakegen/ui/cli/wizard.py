...
async def run_wizard() -> dict[str, Any]:
    ...
    console.print(
        "The wake word is the phrase that activates your device (e.g., 'Hey Computer')."
    )

    wake_word = Prompt.ask(
        "[green]Enter your wake word(s)[/green]", default="hey computer"
    )
    ...
    count = IntPrompt.ask("[green]Number of samples[/green]", default=50)
    config["count"] = count
    ...
    provider_choice = IntPrompt.ask(
        "[green]Select provider[/green]", choices=["1", "2"], default=1
    )
    ...
    output_dir = Prompt.ask("[green]Output directory[/green]", default="./output")
    ...
    console.print(
        "Augmentation adds noise and effects to make the model work in real environments."
    )

    use_augmentation = Confirm.ask("[green]Enable augmentation?[/green]", default=True)
    config["use_augmentation"] = use_augmentation

    if use_augmentation:
        console.print(
            "  [dim]We'll use the default noise profile (background noise, reverb).[/dim]"
        )
