...
class GPUManager:
    ...
    def __init__(self) -> None:
        ...
        self._status: GPUStatus | None = None
    ...
    def assign_model(
        ...
        if required_memory_mb is None:
            required_memory_mb = self.MODEL_MEMORY_ESTIMATES.get(
                model_id.lower(), self.MODEL_MEMORY_ESTIMATES["default"]
            )
        ...
        self._reserved_memory[gpu_id] = (
            self._reserved_memory.get(gpu_id, 0) + required_memory_mb
        )
    ...
    def release_model(self, model_id: str) -> None:
        ...
        if model_id in self._model_assignments:
            ...
            self._reserved_memory[gpu_id] = max(
                0, self._reserved_memory.get(gpu_id, 0) - memory
            )
            logger.debug(f"Released {model_id} from GPU {gpu_id}")

    def _find_best_gpu(self, required_memory_mb: float) -> int | None:
