...
class ASRVerificationConfig(BaseModel):
    ...
    max_word_error_rate: float = Field(
        default=0.3, description="Maximum acceptable word error rate"
    )
    ...
    temperature: float = Field(
        default=0.0, description="Temperature for sampling (0.0 = greedy)"
    )
...
async def verify_pronunciation(
    ...
    expected_text: str,
    config: ASRVerificationConfig | None = None,
    ...
    try:
        ...
        is_correct = (
            word_error_rate <= config.max_word_error_rate
        ...
        if not is_correct:
            if word_error_rate > config.max_word_error_rate:
                error_message = (
                    f"High WER: {word_error_rate:.3f} > {config.max_word_error_rate}"
                )
            if confidence < config.min_confidence:
                error_message = (
                    f"Low confidence: {confidence:.3f} < {config.min_confidence}"
                )
        ...
        raise ASRVerificationError(f"ASR verification failed: {e!s}") from e
...
async def _load_whisper_model(config: ASRVerificationConfig) -> Any:
    ...
    model = whisper.load_model(
        config.model_size, device="cuda" if config.use_gpu else "cpu"
    )
