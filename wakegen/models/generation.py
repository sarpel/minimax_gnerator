...
class GenerationRequest(BaseModel):
    ...
    provider: ProviderType = Field(
        ProviderType.EDGE_TTS, description="The TTS provider to use"
    )
...
class GenerationResponse(BaseModel):
    ...
    samples: list[AudioSample] = Field(
        default_factory=list, description="List of generated audio samples"
    )
    total_duration: float = Field(
        0.0, description="Total duration of all generated samples in seconds"
    )
    success_count: int = Field(
        0, description="Number of successfully generated samples"
    )
...
class GenerationParameters(BaseModel):
    ...
    emphasis_positions: list[int] = Field(
        default_factory=list, description="Word positions to emphasize"
    )
