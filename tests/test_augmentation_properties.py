...
def create_test_audio_file(filepath: str, duration_sec: float = 0.5, sample_rate: int = 16000) -> str:
...
async def test_batch_augment_all_invalid():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        assert len(results) == 0, f"Expected 0 results for invalid files, got {len(results)}"
