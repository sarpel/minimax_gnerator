...
class TestChatTTSProviderVoiceListingProperty:
    """
    Property-based tests for ChatTTSProvider voice listing.
    
    **Feature: critical-bug-fixes, Property 2: Provider voice listing returns valid Voice objects**
    **Validates: Requirements 3.3**
    
    Property: For any provider (ChatTTSProvider), when list_voices is called, 
    all returned Voice objects should have a non-empty `id` attribute that 
    matches the expected voice identifier format.
    """

    @pytest.mark.asyncio
    async def test_chattts_provider_instantiation(self):
        """Test that ChatTTSProvider can be instantiated without errors."""
        ...
        # Test instantiation with explicit config
        config = ProviderConfig()
        provider_with_config = ChatTTSProvider(config=config)
        assert provider_with_config.config is config

    @pytest.mark.asyncio
    @given(language=st.sampled_from(["en", "zh", "multi", None]))
    @settings(max_examples=100, deadline=None)
    async def test_chattts_voice_listing_returns_valid_voices(self, language):
        """
        **Feature: critical-bug-fixes, Property 2: Provider voice listing returns valid Voice objects**
        **Validates: Requirements 3.3**
        ...
        all returned Voice objects should have:
        - A non-empty `id` attribute
        - A valid `provider` attribute set to ProviderType.CHATTTS
        - Valid `gender` and `language` attributes
        """
        from wakegen.providers.opensource.chattts import ChatTTSProvider
        ...
        for voice in voices:
            ...
            # Check provider is set correctly
            assert voice.provider == ProviderType.CHATTTS, f"Voice provider should be CHATTTS, got {voice.provider}"
    ...
    @pytest.mark.asyncio
    async def test_chattts_voice_count(self):
        """Test that ChatTTS returns expected number of voices."""
        from wakegen.providers.opensource.chattts import ChatTTSProvider
    ...
    async def test_chattts_all_voices_have_unique_ids(self):
        ...
        assert len(voice_ids) == len(set(voice_ids)), "All voice IDs should be unique"
